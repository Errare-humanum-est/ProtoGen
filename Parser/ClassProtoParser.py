...
########################################################################################################################
    ...
    def checkAllStatesReachable(self):
        ...
        for cache in self.cacheNode:
            ...
            for key, val in self.cacheNode.get(cache).getvariables().items():
                if val == "INITSTATE_":
            ...
            if not self._dfsExplore(cache, start_state_name):
                return False

        # now do the same for the directories
        for dir in self.dirNode:
            start_state_name = None

            for key, val in self.dirNode.get(dir).getvariables().items():
                if val == "INITSTATE_":
                    start_state_name = key
                    break
            ...
            # Explore the state space via depth-first search
            if not self._dfsExplore(dir, start_state_name):
                return False
